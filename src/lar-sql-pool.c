...
#define LAR_TYPE_SQL_POOL (lar_sql_pool_get_type())
#define LAR_SQL_POOL(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), LAR_TYPE_SQL_POOL, LarSqlPool))
#define LAR_SQL_POOL_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), LAR_TYPE_SQL_POOL, LarSqlPoolClass))
#define LAR_IS_SQL_POOL(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), LAR_TYPE_SQL_POOL))
#define LAR_IS_SQL_POOL_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), LAR_TYPE_SQL_POOL))
#define LAR_SQL_POOL_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), LAR_TYPE_SQL_POOL, LarSqlPoolClass))

typedef struct _LarSqlPoolClass   LarSqlPoolClass;
typedef struct _LarSqlPool        LarSqlPool;
typedef struct _LarSqlPoolPrivate LarSqlPoolPrivate;

struct _LarSqlPoolClass {
...
struct _LarSqlPool {
    ...
    LarSqlPoolPrivate *priv;
};

GType lar_sql_pool_get_type(void) G_GNUC_CONST;

struct _LarSqlPoolPrivate {
    guint                 port;
    gchar *               adresse;
...
static LarSqlPool *LAR_POOL = NULL;

G_DEFINE_TYPE(LarSqlPool, lar_sql_pool, G_TYPE_OBJECT);
static void lar_sql_pool_disconnect(LarSqlPool *pool) { mongoc_client_pool_destroy(pool->priv->pool); }

static void lar_sql_pool_init(LarSqlPool *lar_sql_pool) {
    lar_sql_pool->priv         = G_TYPE_INSTANCE_GET_PRIVATE(lar_sql_pool, LAR_TYPE_SQL_POOL, LarSqlPoolPrivate);
    lar_sql_pool->priv->pool   = NULL;
    lar_sql_pool->priv->dbname = NULL;
    lar_sql_pool->priv->adress = NULL;
    lar_sql_pool->priv->app    = NULL;

    /* TODO: Add initialization code here */
}

static void lar_sql_pool_finalize(GObject *object) {
    LarSqlPool *pool = LAR_SQL_POOL(object);
    lar_sql_pool_disconnect(pool);
    if (pool->priv->adress) g_free(pool->priv->adress);
    if (pool->priv->dbname) g_free(pool->priv->dbname);
    G_OBJECT_CLASS(lar_sql_pool_parent_class)->finalize(object);
}

static void lar_sql_pool_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    g_return_if_fail(LAR_IS_SQL_POOL(object));
    LarSqlPool *pool = LAR_SQL_POOL(object);
    switch (prop_id) {
    case PROP_PORT:
...
static void lar_sql_pool_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) {
    g_return_if_fail(LAR_IS_SQL_POOL(object));
    LarSqlPool *pool = LAR_SQL_POOL(object);
    switch (prop_id) {
    case PROP_PORT:
...
static void lar_sql_pool_class_init(LarSqlPoolClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);

    g_type_class_add_private(klass, sizeof(LarSqlPoolPrivate));

    object_class->finalize     = lar_sql_pool_finalize;
    object_class->set_property = lar_sql_pool_set_property;
    object_class->get_property = lar_sql_pool_get_property;
...
static gboolean lar_sql_pool_connect(LarSqlPool *pool) {
    mongoc_uri_t *uri;
...
void lar_sql_pool_initialize(const gchar *adress, guint port, const gchar *dbname, const gchar *app) {
    if (LAR_POOL != NULL) {
        g_warning("Lar mongo DB pool alredy initialized");
        return;
    }
    LarSqlPool *pool = g_object_new(LAR_TYPE_SQL_POOL, "port", port, "adress", adress, "db-name", dbname, "app-name", app, NULL);
    lar_sql_pool_connect(pool);
}

GObject *lar_sql_pool_find_document(const gchar *collname, const gchar *id, GType otype) {
...
GList *lar_sql_pool_find_licenses(const gchar *jq, guint skip, guint limit) {
    g_return_val_if_fail(LAR_POOL != NULL, NULL);
