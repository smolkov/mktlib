...
G_DEFINE_TYPE_WITH_CODE(LarDevice, lar_device, G_TYPE_OBJECT, G_IMPLEMENT_INTERFACE(JSON_TYPE_SERIALIZABLE, json_serializable_iface_init));



static JsonNode *lar_device_serialize_property(JsonSerializable *serializable, const gchar *name, const GValue *value, GParamSpec *pspec) {
...
gboolean lar_device_deserialize_property(
...
static void json_serializable_iface_init(gpointer g_iface) {
    ...
    iface->serialize_property   = lar_device_serialize_property;
    iface->deserialize_property = lar_device_deserialize_property;
}

static void lar_device_init(LarDevice *lar_device) {
    lar_device->priv = G_TYPE_INSTANCE_GET_PRIVATE(lar_device, LAR_TYPE_DEVICE, LarDevicePrivate);

    /* TODO: Add initialization code here */
}

static void lar_device_finalize(GObject *object) {
    LarDevice *device = LAR_DEVICE(object);
    ...
    G_OBJECT_CLASS(lar_device_parent_class)->finalize(object);
}

static void lar_device_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    g_return_if_fail(LAR_IS_DEVICE(object));
    LarDevice *device = LAR_DEVICE(object);
    switch (prop_id) {
    case PROP_ID:
...
static void lar_device_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) {
    g_return_if_fail(LAR_IS_DEVICE(object));
    LarDevice *device = LAR_DEVICE(object);
...
static void lar_device_class_init(LarDeviceClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);

    g_type_class_add_private(klass, sizeof(LarDevicePrivate));

    object_class->finalize     = lar_device_finalize;
    object_class->set_property = lar_device_set_property;
    object_class->get_property = lar_device_get_property;
...
const gchar *lar_device_get_id(LarDevice *device) {
    ...
    g_return_val_if_fail(LAR_IS_DEVICE(device), NULL);
...
static gboolean lar_device_check_json_object(JsonObject *object) {
...
LarDevice *lar_device_new_from_json(const gchar *json_body) {
    ...
    if (error) {
        g_warning("JSon Parser(%s) decode error %d - %s", g_type_name(LAR_TYPE_DEVICE), error->code, error->message);
        g_error_free(error);

    } else {
        JsonNode *jnode = json_parser_get_root(parser);
        if (JSON_NODE_HOLDS_OBJECT(jnode) && lar_device_check_json_object(json_node_get_object(jnode))) {
